...
//テキストボックスを隠す
userentry.onkeydown = function(event){
    //エンターキーを押したとき
    if(event.keyCode === 13 && userentry.value.length > 0) {
    $("div:hidden").show(1500);
        //ユーザーIDの管理
        ws.send(JSON.stringify({
            type: 'username',
            user: userentry.value,
            text: userentry.value
        }));
...
/*userentry.onkeydown = function(event){
    //エンターキーを押したとき
    if(event.keyCode === 13 && userentry.value.length > 0) {
        $("#userentry").attr('disabled',true)
    }else{
        $("#userentry").attr('disabled',false)
    };
};*/
...
ws.onmessage = function(event) {
    ...
    if (data.type === 'join') {
        ...
        item.addClass('alert alert-info')
            .prepend('<button type="button" class="close" data-dismiss="alert">×</button>')
            .children('div').children('i').after(userName + 'が' + data.user + 'と名前を決めました');
            //userName = data.user;
